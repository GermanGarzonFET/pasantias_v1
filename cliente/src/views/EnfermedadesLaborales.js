...
const TableEnfermedadLaboral=()=>{
    ...
    const getData=()=>{
        ...
        axios.get(API_BASE_URL+'enfermedadesData',{
            headers:payload
          })
    ...
    const peticionDelete=async(id)=>{
        await axios.delete(baseUrl+`enfermedadesData/${id}`)
        .then(response=>{
...
<div className={styles.modal}>
          <div align="right">
            <h3>Opciones </h3>
            <Link
                to={`/detallerEnfermedades/${userSeleccionado._id}`}
            >
                <IconButton>
    ...
    return(
        ...
        <div className="row">
            ...
            <Button variant="contained" color="primary" href="/addEnfermedad">agregar</Button>

            <MateriaTable title="Enfermedades Laborales"
            data={data}
            columns={columns}
            actions={[
                {
